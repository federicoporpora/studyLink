...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/modello-del-dominio-utente.png}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/diagramma-dei-package.png}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/diagramma-dei-package-registrazione-autenticazione.png}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/diagramma-dei-package-profilo.png}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/diagramma-dei-package-evento.png}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/diagramma-dei-package-bacheca.png}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/diagramma-dei-package-comunicazione.png}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/interazione-registrazione.png}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/interazione-autenticazione.png}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/interazione-elimina-utente.png}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/interazione-modifica-profilo.png}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/interazione-creazione-evento.png}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/interazione-gestione-evento.png}
\end{figure}
...
\subsection{Requisiti non funzionali}
Dalla tabella dei vincoli si nota che due requisiti non funzionali fondamentali sono l'usabilità e la sicurezza. Questi due aspetti si trovano parzialmente in contrasto tra loro.
Per gestire questo problema, sono state prese le seguenti decisioni architetturali:
\begin{itemize}
    \item Per garantire la sicurezza, si è scelto di utilizzare tecnologie standard che proteggono le comunicazioni (come il protocollo HTTPS/TLS) e gestiscono in modo sicuro le autenticazioni.
    \item Per favorire l'usabilità (in particolare la possibilità di usare l'app da dispositivi diversi), si è deciso di salvare i dati in un database remoto e non in locale sul dispositivo dell'utente. Questa scelta diminuisce in parte la sicurezza, poiché i dati sono più esposti, ma è essenziale per la portabilità.
\end{itemize}

\subsection{Scelta dell'architettura}
Per soddisfare le richieste del cliente e le scelte appena descritte, l'applicazione verrà sviluppata seguendo un'architettura client/server a tre livelli:
\begin{itemize}
    \item Il \textbf{client} gestisce la presentazione grafica e l'interazione con l'utente. Per semplificare lo sviluppo e rendere il sistema più flessibile, si è optato per un \textit{thin client}.
    \item Il \textbf{server} elabora la logica di business e controlla la validità dei dati inviati dal client. Comunica con il client fornendo delle API REST e si occupa di interrogare il database.
    \item Il \textbf{livello di persistenza} ha il compito di memorizzare e strutturare le informazioni. Per semplificare l'accesso ai dati, si è deciso di usare la tecnica dell'Object-Relational Mapping (ORM).
\end{itemize}
Infine, per ridurre la possibilità di errori e facilitare futuri ampliamenti, i componenti del server sono stati progettati \textit{senza stato} (stateless).

\subsection{Scelte tecnologiche}
Per costruire l'applicazione sono state impiegate diverse tecnologie. Per separare bene i compiti e facilitare la manutenzione del codice, si è scelto di adottare il pattern Model-View-Controller (MVC). 
Nello specifico, il server sarà implementato usando il framework \textbf{ASP.NET Core} e il linguaggio \textbf{C\#}. Questa soluzione offre una struttura stabile e include librerie pronte all'uso per gestire la sicurezza (come i \textbf{JSON Web Token} per l'autenticazione) e le comunicazioni in tempo reale (come \textbf{SignalR} per la chat).
Per il salvataggio dei dati si è scelto il database relazionale \textbf{PostgreSQL}, collegato tramite l'ORM \textbf{Entity Framework Core}, che si integra in modo nativo con ASP.NET. 
Infine, per sviluppare il lato client è stata scelta la libreria \textbf{React}, che permette di creare facilmente interfacce utente dinamiche, veloci e adatte anche ai dispositivi mobili.
