...
È da notare che:
\begin{itemize}[leftmargin=*]
    \item \textbf{Ereditarietà da una classe base comune}: Tutti i controller condividono una classe base comune \texttt{Controller}, la quale detiene il riferimento protetto al contesto del database (\texttt{\# contesto: ContestoStudyLink}). In questo modo, l'accesso alle tabelle tramite ORM è centralizzato ed ereditato in modo uniforme.
    \item \textbf{Classe astratta} \texttt{ControllerAttore}: È stata introdotta la classe astratta \texttt{ControllerAttore} (che estende la classe base \texttt{Controller}) come superclasse per \texttt{ControllerRegistrazione} e \texttt{ControllerAccount}, centralizzando le logiche di validazione legate all'anagrafica degli attori.
    \item \textbf{Incapsulamento e sicurezza}: All'interno di \texttt{ControllerAttore} sono definiti esclusivamente come privati (\texttt{-}) i metodi di validazione sensibili: \texttt{verificaUnicitàEmail()}, \texttt{verificaSintassiEmail()} e \texttt{verificaComplessitàPassword()}. Questa soluzione assicura che tali metodi critici non siano accessibili o invocabili dall'esterno dell'architettura dei controller, garantendo una maggiore sicurezza ed evitando la duplicazione di codice.
    \item \textbf{Disaccoppiamento tramite Interfacce}: Ogni controller concreto implementa la sua rispettiva interfaccia (come ad esempio \texttt{IControllerRegistrazione} e \texttt{IControllerAccount}), garantendo il disaccoppiamento tra la definizione delle API del server e la loro effettiva implementazione.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    ...
    \item aggiungere funzione bloccaAccount e pensare a come fare un account bloccato
    \item controllare dove ho messo la funzione modificaPassword, vedere se è nel controller o nel modello
    \item passare a modificaPassword anche la password vecchia
    \item aggiungere la funzione eliminaAccount in contesto studylink (???)
    \item controllare le interfacce della progettazione (arancioni) e vedere se sono tutte
\end{itemize}
