...
\begin{tabularx}{\textwidth}{|p{2cm}|X|p{3cm}|}
    ...
    \textbf{ID} & \textbf{Requisito} & \textbf{Tipo} \\ \hline
    \endhead
        {R01N}R01N & La piattaforma deve essere accessibile online & Non funzionale \\ \hline
        {R02N}R02N & Le Credenziali di un Utente sono univoche & Non funzionale \\ \hline
        {R03N}R03N & L'interfaccia della Bacheca deve essere essenziale ed intuitiva & Non funzionale \\ \hline
        {R04N}R04N & Il fuso orario usato dall'applicazione è GMT+1 & Non funzionale \\ \hline
        {R05N}R05N & Il formato di data e orario è gg/mm/aa e hh:mm 24h & Non funzionale \\ \hline
        {R06N}R06N & L'applicazione deve rispettare le norme definite nel \href{https://gdpr-info.eu/}{GDPR} & Non funzionale \\ \hline
        {R07N}R07N & La Bacheca globale deve essere costantemente aggiornata & Non funzionale \\ \hline
        {R08N}R08N & La Password scelta dall'Utente deve essere di almeno 8 caratteri e contenere almeno un numero e un carattere speciale (\#@?!\_-) & Non funzionale \\ \hline
        {R09N}R09N & La ricezione ed invio dei Messaggi deve essere istantanea & Non funzionale \\ \hline
\end{tabularx}
...
\begin{tabularx}{\textwidth}{|X|X|}
    ...
    \textcolor{red}{Cifratura dell Comunicazioni} & \begin{itemize}[nosep, leftmargin=*]
                    \item Punto 1;
                    \item Punto 2
                \end{itemize} \\ \hline
\end{tabularx}
...
Per evitare problemi di impaginazione dovuti alla verbosità degli scenari, i casi d'uso di sicurezza sono presentati in formato testuale strutturato.

\subsubsection*{Controllo Accessi}
\begin{itemize}[leftmargin=*]
    \item \textbf{Descrizione:} Gli accessi al sistema e alle funzioni sensibili devono essere controllati.
    \item \textbf{Misuse case:} Furto credenziali, Violazione dei permessi.
    \item \textbf{Relazioni:} Autenticazione, Gestione Evento.
    \item \textbf{Precondizione:} L'attaccante conosce l'Email Istituzionale di un Utente e tenta di forzarne la password.
    \item \textbf{Postcondizione:} Il sistema blocca l'accesso e traccia il tentativo anomalo nel Log.
    \item \textbf{Scenario Principale (L'app si difende):} L'attaccante tenta di accedere inserendo password errate. Il sistema controlla le credenziali. Al terzo tentativo errato consecutivo, il sistema blocca temporaneamente l'accesso per quell'Account e registra il tentativo nel Log di sicurezza. L'attaccante non entra.
    \item \textbf{Scenario di attacco avvenuto con successo:} L'attaccante accede al sistema con credenziali corrette (ottenute tramite phishing). Il sistema consente l'accesso. L'attaccante naviga nel sistema in modo fraudolento. Il sistema registra in modo inalterabile tutte le operazioni dell'utente nel Log, permettendo la successiva indagine.
\end{itemize}

\vspace{0.5cm}

\subsubsection*{Confidenzialità e Protezione Dati}
\begin{itemize}[leftmargin=*]
    \item \textbf{Descrizione:} Riservatezza dei dati durante il trasferimento tra Client e Server.
    \item \textbf{Misuse case:} Sniffing dati, Man in the Middle.
    \item \textbf{Relazioni:} Utilizzo Chat, Visualizzazione Bacheca e Partecipante.
    \item \textbf{Precondizione:} L'attaccante ha i mezzi per intercettare il traffico di rete (es. connesso alla stessa Wi-Fi pubblica/universitaria).
    \item \textbf{Postcondizione:} L'attaccante non ottiene nessuna informazione sensibile.
    \item \textbf{Scenario Principale (L'app si difende):} Il sistema invia messaggi crittografati sulla rete per fornire i suoi servizi (es. messaggi della Chat). L'attaccante intercetta i pacchetti ma non riesce a decifrarne il contenuto grazie alla cifratura HTTPS/TLS. Se il pacchetto viene manipolato, la firma non corrisponde e viene scartato dal sistema.
    \item \textbf{Scenario di attacco avvenuto con successo:} L'attaccante riesce a forzare la decifratura e altera le informazioni in transito. Il sistema riceve i dati alterati e li elabora, ma ne trascrive il contenuto, il mittente e l'orario nel Log di sistema per permettere la rilevazione dell'anomalia.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Descrizione:} I dati persistenti e le regole del sistema non devono poter essere modificati da utenti non autorizzati.
    \item \textbf{Misuse case:} Furto Credenziali, Escalation dei privilegi.
    \item \textbf{Relazioni:} Gestione Evento, Rimozione Partecipante, Annullamento Prenotazione.
    \item \textbf{Precondizione:} L'attaccante invia richieste API alterate al server per manipolare un Evento di cui non è Organizzatore.
    \item \textbf{Postcondizione:} Il sistema rileva l'azione non autorizzata e preserva l'integrità del database.
    \item \textbf{Scenario Principale (L'app si difende):} L'attaccante tenta di modificare o cancellare un Evento altrui. Il sistema verifica i permessi a livello server, rileva che l'utente non è autorizzato (non è l'Organizzatore), respinge la richiesta e logga il tentativo di manipolazione.
    \item \textbf{Scenario di attacco avvenuto con successo:} L'attaccante, agendo con le credenziali rubate dell'Organizzatore, cancella l'Evento in modo illegittimo. Il sistema esegue l'azione credendola legittima, invia le notifiche automatiche ai Partecipanti (che si accorgeranno dell'anomalia) e memorizza le modifiche nel Log inalterabile, permettendo all'amministratore di risalire all'azione illecita.
\end{itemize}
...
\begin{enumerate}
    \item Creazione di un log inalterabile per tracciare alcune azioni sensibili come:
    \begin{itemize}
        ...
        \item Operazioni amministrative e critiche relative al backend.
    \end{itemize}
\end{enumerate}
...
\begin{tabularx}{\textwidth}{|p{2cm}|X|p{3cm}|}
    ...
    \textbf{ID} & \textbf{Requisito} & \textbf{Tipo} \\ \hline
    \endhead
    R22F & Il sistema registra le operazioni sensibili e i tentativi di accesso in un file di log. & Funzionale \\ \hline
    R23F & I log di sistema possono essere inviati a un sistema esterno per l'analisi e conservazione. & Funzionale \\ \hline
    R10N & La comunicazione tra client e server deve essere obbligatoriamente cifrata tramite protocollo HTTPS/TLS. & Non funzionale \\ \hline
    R11N & Dopo 3 tentativi di Autenticazione falliti consecutivi, il sistema blocca l'accesso a quell'account per 15 minuti. & Non funzionale \\ \hline
\end{tabularx}
    ...
    \textbf{Scenari alternativi} & 
        \begin{itemize}[nosep, leftmargin=*]
            ...
            \begin{enumerate}[nosep, leftmargin=*]
                ...
                \item \textcolor{red}{[NUOVO]} Se il numero di tentativi errati supera i 3 consecutivi, il sistema blocca temporaneamente l'accesso per 15 minuti e registra l'evento nel Log.
            \end{enumerate}
        \end{itemize} \\ \hline
    \textbf{Requisiti non funzionali} & \hyperref[R03N]{R03N}, \hyperref[R07N]{R07N}, \hyperref[R09N]{R09N}, \textcolor{red}{[NUOVO] R10N, R11N} \\ \hline
    \textbf{Punti aperti} & \\ \hline
\end{tabularx}
