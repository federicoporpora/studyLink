...
const UserProfile = () => {
  ...
  const fetchProfile = async () => {
    try {
      ...
      const res = await fetch(`${apiUrl}/api/profilo/${id}`, {
        headers: {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100dvh', width: '100%', backgroundColor: '#f9f9f9' }}>
      ...
      <div style={{ backgroundColor: 'var(--primary)', color: 'white', padding: '15px 20px', display: 'flex', alignItems: 'center', boxShadow: '0 2px 5px rgba(0,0,0,0.1)' }}>
        ...
        <h2 style={{ fontSize: '18px', fontWeight: '500' }}>Profilo Utente</h2>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
        ...
        {/* Comments Section */}
        <h3 style={{ fontSize: '18px', color: 'var(--text-dark)', marginBottom: '15px', fontWeight: '600' }}>Commenti ({profile.commenti?.length || 0})</h3>
        
        {!profile.commenti || profile.commenti.length === 0 ? (
          <p style={{ color: 'var(--text-light)', textAlign: 'center', padding: '20px', backgroundColor: 'white', borderRadius: '10px' }}>Nessun commento ricevuto.</p>
        ) : (
